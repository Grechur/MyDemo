 ...
 * @Description: 最长回文子串
 * @Author: Grechur
 * @CreateDate: 2019/11/6 11:02
 * @UpdateUser: Grechur
 * @UpdateDate: 2019/11/6 11:02
 */
public class LongestPalindromic {

    public static void main(String[] args) {
        System.out.println(longestPalindromeVio("acbbca"));
        System.out.println(longestPalindromeManacher("acbbca"));
        System.out.println(longestPalindromeDynamic("ddacca"));
    }

    /**
     * 暴力解法，第一次看到想到的方法，截取字符串，判断字符串是否是回文
     * 时间复杂度高
     * @param s
     * @return
     */
    ...
    public static boolean isPalindrome(String s){
        int length = s.length();//"abcba"
        for (int i = 0; i < length / 2; i++) {
            if(s.charAt(i)!=s.charAt(length-i-1)){//判断字符串左右两边是否一致
                return false;
            }
        }
        return true;
    ...
    public static String longestPalindromeDynamic(String s) {
        ...
        // 左边界一定要比右边界小，因此右边界从 1 开始      dfsgfdacbbcarterere
        for (int r = 1; r < len; r++) {
            for (int l = 0; l < r; l++) {
                // 区间应该慢慢放大
